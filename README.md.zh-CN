...
│       │   ├── ai/                      # AI 模块
│       │   │   ├── providers/           # AI 提供商实现
│       │   │   ├── EmbeddingClient.kt   # Embedding 客户端接口
│       │   │                              + Ollama 实现
│       │   │   ├── SemanticMatcher.kt   # 语义相似度匹配
│       │   │                             （余弦相似度）
│       │   │   ├── QueryAnalyzer.kt     # 代码关键词分析器
│       │   │                             （标签检索）
│       │   │   ├── RuleDoc.kt           # RuleDoc 数据类（规则
│       │   │                              知识文档，含 embedding
│       │   │                              缓存）
│       │   │   ├── AiFinding.kt         # AI 发现结果 +
│       │   │                              FindingMetadata
│       │   │   ├── AiReviewContext.kt   # AI 审查上下文
│       │   │   ├── RuleDocLoader.kt     # RuleDoc 加载器（内置 +
│       │   │                              用户自定义）
│       │   │   ├── PromptAssembler.kt   # 分层提示词组装器
│       │   │                             （支持三种检索模式）
│       │   │   └── AiConfig.kt          # AI 配置（含检索模式
│       │   │                              设置）
│       │   ├── gui/                     # Compose 图形界面
│       │   ├── cli/                     # 命令行入口
│       │   └── report/                  # 报告生成
│       └── resources/
│           ├── ai_rules/                 # AI RuleDoc 规则知识
│           │                              文档
│           │   ├── system-prompt.md     # System Prompt（固定格式
│           │   │                              规则）
│           │   ├── task-diff.md         # Task Prompt（Diff 模式任务
│           │   │                              说明）
│           │   ├── task-global.md       # Task Prompt（Global 模式任务
│           │   │                              说明）
│           │   ├── common/              # 通用规则
│           │   │   ├── java/            # Java 通用规则文档
│           │   │   └── android/         # Android 通用规则文档
│           ├── ai_prompts/              # 兼容保留：原提示词
│           │                              模板（已迁移）
│           │   └── common/              # 通用提示词
...
- ✅ **GUI 新增三个设置对话框**：
  ...
  - 本地规则设置：勾选启用/禁用单个本地规则，独立开关本地规则审查
